...
public class JpegThumbnail {
	 ...
	 * @throws JpegThumbnailException
	 */
	public void setScale(double scale) throws JpegThumbnailException {
		if (scale <= 0) {
			throw new JpegThumbnailException(" 缩放比例不能为 0 和负数！ ");
		}
	 ...
	 * @throws JpegThumbnailException
	 */
	public void setSmallWidth(int smallpicwidth) throws JpegThumbnailException {
		if (smallpicwidth <= 0) {
			throw new JpegThumbnailException(" 缩影图片的宽度不能为 0 和负数！ ");
		}
		this.resetJpegToolParams();
	...
	public void setSmallHeight(int smallpicheight) throws JpegThumbnailException {
		if (smallpicheight <= 0) {
			throw new JpegThumbnailException(" 缩影图片的高度不能为 0 和负数！ ");
		}
		this.resetJpegToolParams();
	...
	public void doFinal(String pic_big_pathfilename,
			String pic_small_pathfilename) throws JpegThumbnailException {
		if (!this.isInitFlag) {
			throw new JpegThumbnailException(" 对象参数没有初始化！ ");
		}
		if (pic_big_pathfilename == null || pic_small_pathfilename == null) {
			throw new JpegThumbnailException(" 包含文件名的路径为空！ ");
		}
		if ((!pic_big_pathfilename.toLowerCase().endsWith("jpg"))
				&& (!pic_big_pathfilename.toLowerCase().endsWith("jpeg"))) {
			throw new JpegThumbnailException(" 只能处理 JPG/JPEG 文件！ ");
		}
		...
		try {
			...
			throw new JpegThumbnailException(" 读取源图像文件出错！ ");
		}
...
//		double scale = (double) pic_big_width / pic_big_height;// 图像的长宽比例
		if (this.smallpicwidth != 0) {// 根据设定的宽度求出长度
			...
			throw new JpegThumbnailException(" 对象参数初始化不正确！ ");
		}
		...
		try {
			...
			throw new JpegThumbnailException(" 写入缩略图像文件出错！ ");
		}
	}
}